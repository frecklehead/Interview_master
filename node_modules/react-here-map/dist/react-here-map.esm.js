...
const PlatformContext = /*#__PURE__*/createContext({
  platform: null
});
...
const buildMap = (platform, options) => {
  ...
  if (container && build && retObject.options) {
    ...
    const mapLayer = getProperty(defaultLayers, retObject.options.mapType);
    // Create a Map
...
const HMap = props => {
  ...
  const containerRef = props.ref || useRef(null);
  useEffect(() => {
  ...
  const loading = loadingEl || React.createElement(LoadingComponent, null);
  return (
    // only render map provider when there is a platform state
    platformState.platform && React.createElement(MapContext.Provider, {
      value: mapState
...
const HMapPolyline = props => {
  ...
  return React.createElement(BaseMapObject, {
...
const HMapLayer = props => {
  ...
  useEffect(() => {
    ...
    const mapLayer = getProperty(defaultLayers, type);
    if (mapLayer) {
      var _mapContext$map;
...
const useHPlatform = (platformOptions, children) => React.createElement(HPlatform, {
  options: platformOptions
}, children);

export { HMap, HMapCircle, HMapLayer, HMapMarker, HMapPolygon, HMapPolyline, HMapRectangle, HPlatform, useHPlatform };
//# sourceMappingURL=react-here-map.esm.js.map
